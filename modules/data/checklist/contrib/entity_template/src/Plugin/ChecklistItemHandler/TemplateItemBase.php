...
abstract class TemplateItemBase extends ContextAwareChecklistItemHandlerBase implements IterativeChecklistItemHandlerInterface, StatefulChecklistItemHandlerInterface, ExpectedOutcomeChecklistItemHandlerInterface, ActionStateChecklistItemHandlerInterface, DependentPluginInterface, InteractiveChecklistItemHandlerInterface, ActionOperationsChecklistItemHandlerInterface {
  ...
  /**
   * Checks template applicability after candidate inputs have been assigned.
   */
  protected function templateApplies(Template $template): bool {
    return $template->applies();
  }

  /**
   * Supplies detached initial data, or NULL to let the template create it.
   */
  abstract protected function initialTarget(array &$selection): ?TypedDataInterface;
  ...
  public function expectedOutcomeDefinitions(): array {
    ...
    $definition = EntityDataDefinition::create($type)->setLabel(new TranslatableMarkup('Prepared entity'));
    if (!$unrestricted) {
  ...
  public function actionIteration(ChecklistAttempt $attempt): ChecklistItemResult {
    ...
    $entity = NULL;
    try {
      $initial = $execution->getState() ? NULL : $this->initialTarget($selection);
      $partial = $execution->getState()?->target->getValue();
      if ($partial instanceof FieldableEntityInterface) {
        $this->checkTarget($partial, $selection);
        $this->checkFieldAccess($partial, $editable);
      }
      $result = $template->executeTemplate($initial, $execution);
      ...
      if (!$result->isComplete() || !$entity instanceof FieldableEntityInterface) {
        throw new \UnexpectedValueException('The template did not prepare a fieldable entity.');
      }
    ...
    return $this->completedResult($entity, $editable, $selection);
  }
  ...
  public function getEditorSession(): ?array {
    ...
    $editor = PhpSerialize::decode($stored['snapshot']);
    $this->checkTarget($editor[0]->getDataManager()->getContext('entity')->getContextValue(), $this->selection());
    return $editor;
  }
  ...
  public function editorResult(FormSession $session, array $editable, bool $complete = FALSE, ?array $selection = NULL): ChecklistItemResult {
    ...
    $this->checkTarget($entity, $selection ?? $this->selection());
  ...
  protected function completedResult(FieldableEntityInterface $entity, array $editable, ?array $selection = NULL): ChecklistItemResult {
    $selection ??= $this->selection();
    return new ChecklistItemResult(ChecklistAttempt::SUCCEEDED, outcomes: ['entity' => $entity], persist: function () use ($entity, $editable, $selection): void {
      $this->checkTarget($entity, $selection);
  ...
  public function getActionState(): ?ChecklistActionState {
    if ($this->getItem()->isComplete()) {
      return new ChecklistActionState('complete', (string) $this->t('Entity saved.'));
    }
    if ($this->getItem()->isFailed()) {
      return new ChecklistActionState('failed', (string) $this->t('Template preparation failed.'));
    }
    if (!empty($this->getItem()->get('state')->get('editor')->getValue()['snapshot'])) {
